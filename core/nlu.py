...
class DDxGraphNLU(BaseSymptomRetriever):
    """
    Symptom retrieval engine leveraging SentenceTransformers and cosine
    similarity. Maps unstructured language inputs to structured knowledge
    graph evidence nodes.
    """

    def __init__(self, G, embedding_model_name=EMBEDDING_MODEL):
        """
        Initialize the retrieval engine with a networkx graph and embedding
        model.

        Args:
            G (networkx.Graph): Knowledge graph loaded with DDxPlus data and
              pre-computed embeddings.
            embedding_model_name (str): SentenceTransformer model string for
              encoding.
        """
        with open('Data/ddxplus/release_evidences.compact.json', 'r') as f:
        ...
        for node, data in G.nodes(data=True):
            if data.get("type") == "evidence":
                if "embedding" in data:
                    ...
                    txt = (
                        f"{data.get('question_en', '')} "
                        f"{data.get('question_fr', '')}"
                    )
    ...
    def parse_query1(self, user_query):
        """
        Parse user query into individual spans, verify negations, and query
        similarity search.
        ...
        Returns:
            Tuple[List[dict], List[str], List[Any]]:
                ...
                - List of parsed values (either 'YES'/'NO' or lists of
                  categorical/numerical keys).
        """
        all_findings = []
        delimiters = (
            r'[,;]|\bbut\b|\band\b|\balso\b|\bhowever\b|\bplus\b|\bexcept\b|'
            r'(?<!\d)\.(?!\d)'
        )
        raw_chunks = [
            c.strip()
            for c in re.split(delimiters, user_query.lower())
            if c.strip()
        ]
    ...
    def _find_best_match(self, text, top_k=7):
        ...
        Returns:
            List[dict] or None: List of matching candidates sorted by score
                                within a relative threshold, or None if no
                                match is found.
        """
        ...
        for idx in top_indices:
            ...
            if dtype in ["M", "C"]:
                val_nodes = [
                    v for _, v in self.G.out_edges(eid)
                ...
                if val_nodes:
                    ...
                    if val_embs:
                        ...
                        v_scores = cosine_similarity(
                            query_vec, val_matrix
                        )[0]
                        for i, vs in enumerate(v_scores):
                            if vs > current_candidate['score']:
                                ...
                                if vs >= THRESH_VALUE:
                                    current_candidate['value'].append(
                                        val_ids[i]
                                    )
        ...
        sorted_results = sorted(
            best_results, key=lambda x: x['score'], reverse=True
        )
        k = 0
